...
public class SaBackupAgent extends BackupAgent
{
	...
	{
		Log.d( MainActivity.TAG, "onBackup" );
		SharedPreferences sp = getSharedPreferences( MainActivity.PREFS, MODE_PRIVATE );
		if( sp.contains( MainActivity.KEY ) )
		{
			String val = sp.getString( MainActivity.KEY, null );
			String oldVal = null;
			if( val != null )
			{
	...
	public void onRestore( BackupDataInput data, int appVersionCode,
			ParcelFileDescriptor newState ) throws IOException
	{
		Log.d( MainActivity.TAG, "onRestore" );
		String val = null;
		...
		if( val != null )
		{
			...
			editor.putString( MainActivity.KEY, val );
			editor.commit();
		}
	}
